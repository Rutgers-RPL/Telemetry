...
//   Telem[75] = ' ';
   dtostrf(millis()/1000.0,8,3,Telem+44); //millis() will overflow back to zero after about 50 days
   Telem[52]=' ';
   dtofstr(cam,1,0,Telem+53);
   if(GPS.latitudeDegrees != 0 && sendCounter >= 10){
     Telem[54] = '_'; // used to tell the reciever that gps data has been sent
     dtostrf(GPS.latitudeDegrees,7,4,Telem+55);
     Telem[62] = ' ';
     dtostrf(GPS.longitudeDegrees,7,4,Telem+63);
     rf95.send((uint8_t *)Telem, 69);
